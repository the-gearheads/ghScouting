    ...
    for team, values in d.items():
        ...
        for i in foo:
                ...
                for y in w:
                    ...
                    try:
                        if value == i.processor(values[i.name]):
                            col_data[team][
                                i.args["display"] + "_" + value
                            ] = i.processor(values[i.name])
                        else:
                            col_data[team][i.args["display"] + "_" + value] = None
                    except KeyError:
                        if value == i.processor(values[i.name]):
                            col_data[team][i.name + "_" + value] = i.processor(
                                values[i.name]
                            )
                        else:
                            col_data[team][i.name + "_" + value] = None
            ...
            if issubclass(type(i), scouting.Element.ElementImage):
                continue
            if issubclass(type(i), scouting.Element.ElementDisplayText):
                continue
...
def rank_server(config):
    ...
    return render_template(
        "analysis/rank.html", ranks=Markup(ranks), data=Markup(data), col=Markup(col)
    )
...
def filter_server(config):
    ...
    return render_template(
        "analysis/filter.html",
        data=Markup(data),
        col=Markup(col),
        newCol=Markup(newCol),
    )
        ...
        for val_key in values.keys():
            for datapoint in nameArr:
                if val_key == datapoint:
                    ...
                    if (
                        "_" in val_key
                        and val_key[leng - 1].isdigit() == True
                        and values[val_key] != None
                    ):
